...
export class MetadataToolComponent extends ToolComponent implements OnInit, OnMount {  
    ...
    getLayerInfo(layer: Layer) {
        ...
        this.metadataService.getMetadataById(layer.metadataId).subscribe((metadata: Metadata) => {
            ...
            if (metadata) {
                this.translateService.get("metadata").subscribe(metadataText => {
                    if (metadata.projecao)
                        htmlTable += '<tr> <td><b>' + metadataText.projection + '</b></td><td colspan="2">' + metadata.projecao + '</td></tr>';
                    
                    if (metadata.datumHorizontal)
                        htmlTable += '<tr> <td><b>' + metadataText.horizontalDatum + '</b></td><td colspan="2">' + metadata.datumHorizontal + '</td></tr>';
        
                    if (metadata.fonte)
                        htmlTable += '<tr> <td><b>' + metadataText.dataSource + '</b></td><td colspan="2">' + metadata.fonte + '</td></tr>';
        
                    if (metadata.autor)
                        htmlTable += '<tr> <td><b>' + metadataText.authors + '</b></td><td colspan="2">' + metadata.autor + '</td></tr>';
        
                    if (metadata.informacoesAdicionais)
                        htmlTable += '<tr> <td><b>' + metadataText.additionalInformation + '</b></td><td colspan="2">' + metadata.informacoesAdicionais + '</td></tr>';                    
                });
            }
